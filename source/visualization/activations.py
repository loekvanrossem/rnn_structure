...
class PointAnimation(animation.AnimationSubPlot):
    ...
    def __init__(
        ...
        colors_cycler = cycle(prop_cycle.by_key()["color"])
        self.possible_colors = []
        for n, c in enumerate(colors_cycler):
            self.possible_colors.append(c)
            if n > 100:
                break
...
class ActivationsAnimation(PointAnimation):
    ...
    fixed_points : dict, optional
        Labeled points that are invariant with respect to the epoch
    colors : list[int], optional
        An assignment of colors to the points
